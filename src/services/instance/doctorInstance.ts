  ...
  async (error) => {
    ...
    if (error.response) {
      if (error.response.status === 401 && !originalRequest._retry) {
        console.log(
          'Entered 401 unauthorised response middleware, trying to create new token for doctors'
        );
      ...
      if (
        ...
        console.log(
          'reached doctor axios interceptor toast statement for user errors'
        );
...
async function getNewAccessToken() {
  ...
  return response.data.data.doctorAccessToken;
}
