...
const VideoCallTest = () => {
    ...
    useEffect(() => {
        ...
        if (socket) {           
            ...
            if (peerService.peer) {
                console.log("Entered event tracking in peer connection. ", peerService.peer);
                ...
                peerService.peer.onnegotiationneeded = async () => {
                    ...
                    socket.emit('negotiation-offer', { target: remoteSocketId as string, offer });
                };
            }
        } else {
    ...
    useEffect(() => {
      ...
      }, [localVideoRef.current, localStreamRef.current, remoteVideoRef.current]);
    ...
    const startCall = async () => {
        try {
            ...
            stream.getTracks().forEach((track) => {
                console.log('Adding track to remote peer');
                peerService.peer.addTrack(track, stream);
            });
