...
const UserChatInterface = () => { 
  ...
  const fetchPeople = useCallback(async () => {
    ...
    try {
      ...
      if (response.status) {
        const transformedData = response.data.messages.map(({ _id, doctor, lastMessage, unreadCount }: { _id: string, doctor: IDoctorProfile, lastMessage: Message, unreadCount: number }) => ({
                  id: _id.toString(),
                  name: doctor.name,
